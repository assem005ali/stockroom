...
import {
  ...
  Menu, History, ShoppingCart, TrendingDown
} from "lucide-react";

/* ---------------------------------------------------------------------
   Stockroom — Warehouse Management System
   Data persists via window.storage (per-user, private).
--------------------------------------------------------------------- */
...
const KEYS = {
  warehouses: "stockroom:warehouses",
  shelves: "stockroom:shelves",
  products: "stockroom:products",
  locations: "stockroom:locations",
  sales: "stockroom:sales",
  seeded: "stockroom:seeded",
};
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const seeded = await storageGet(KEYS.seeded);
      if (!seeded) {
        const seed = seedData();
        await Promise.all([
          storageSet(KEYS.warehouses, seed.warehouses),
          storageSet(KEYS.shelves, seed.shelves),
          storageSet(KEYS.products, seed.products),
          storageSet(KEYS.locations, seed.locations),
          storageSet(KEYS.sales, seed.sales),
          storageSet(KEYS.seeded, true),
        ]);
        setWarehouses(seed.warehouses);
        setShelves(seed.shelves);
        setProducts(seed.products);
        setLocations(seed.locations);
        setSales(seed.sales);
      } else {
        const [w, s, p, l, sa] = await Promise.all([
          storageGet(KEYS.warehouses), storageGet(KEYS.shelves), storageGet(KEYS.products), storageGet(KEYS.locations), storageGet(KEYS.sales),
        ]);
        setWarehouses(w || []);
        setShelves(s || []);
        setProducts(p || []);
  ...
  const mutate = {
    ...
    updateProduct: async (id, form) => {
      const next = products.map((p) => (p.id === id ? { ...p, ...form } : p));
      setProducts(next);
      await storageSet(KEYS.products, next);
    },
    deleteProduct: async (id) => {
      const nextP = products.filter((p) => p.id !== id);
      const nextL = locations.filter((l) => l.productId !== id);
      setProducts(nextP);
      setLocations(nextL);
      await Promise.all([storageSet(KEYS.products, nextP), storageSet(KEYS.locations, nextL)]);
    },
    addWarehouse: async (form) => {
      const item = { id: uid("wh"), ...form };
      const next = [...warehouses, item];
      setWarehouses(next);
      await storageSet(KEYS.warehouses, next);
    },
    updateWarehouse: async (id, form) => {
      const next = warehouses.map((w) => (w.id === id ? { ...w, ...form } : w));
      setWarehouses(next);
      await storageSet(KEYS.warehouses, next);
    },
    deleteWarehouse: async (id) => {
      const nextW = warehouses.filter((w) => w.id !== id);
      const removedShelfIds = new Set(shelves.filter((s) => s.warehouseId === id).map((s) => s.id));
      const nextS = shelves.filter((s) => s.warehouseId !== id);
      const nextL = locations.filter((l) => l.warehouseId !== id && !removedShelfIds.has(l.shelfId));
      setWarehouses(nextW); setShelves(nextS); setLocations(nextL);
      await Promise.all([storageSet(KEYS.warehouses, nextW), storageSet(KEYS.shelves, nextS), storageSet(KEYS.locations, nextL)]);
    },
    addShelf: async (warehouseId, form) => {
      const item = { id: uid("sh"), warehouseId, ...form };
      const next = [...shelves, item];
      setShelves(next);
      await storageSet(KEYS.shelves, next);
    },
    updateShelf: async (id, form) => {
      const next = shelves.map((s) => (s.id === id ? { ...s, ...form } : s));
      setShelves(next);
      await storageSet(KEYS.shelves, next);
    },
    deleteShelf: async (id) => {
      const nextS = shelves.filter((s) => s.id !== id);
      const nextL = locations.filter((l) => l.shelfId !== id);
      setShelves(nextS); setLocations(nextL);
      await Promise.all([storageSet(KEYS.shelves, nextS), storageSet(KEYS.locations, nextL)]);
    },
    addLocation: async (form) => {
      const item = { id: uid("loc"), ...form };
      const next = [...locations, item];
      setLocations(next);
      await storageSet(KEYS.locations, next);
    },
    updateLocation: async (id, form) => {
      const next = locations.map((l) => (l.id === id ? { ...l, ...form } : l));
      setLocations(next);
      await storageSet(KEYS.locations, next);
    },
    ...
    addSale: async (productId, form) => {
      ...
      const nextProducts = products.map((p) =>
        p.id === productId ? { ...p, total_quantity: Math.max(0, Number(p.total_quantity || 0) - Number(form.quantity || 0)) } : p
      );
      setSales(nextSales);
      setProducts(nextProducts);
      await Promise.all([storageSet(KEYS.sales, nextSales), storageSet(KEYS.products, nextProducts)]);
    },
  };
